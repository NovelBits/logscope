...
import { lookupHciError, lookupCompanyId } from "@novelbits/ble-spec";
import {
  COLOR_ERROR,
  field,
  fieldWithSpec,
...
import { attDecoders } from "./att-decoders";
...
// ---------------------------------------------------------------------------
// ATT opcode dispatcher
// ---------------------------------------------------------------------------

/**
 * Decode an ATT read/write/notify/mtu opcode, mutating fields and returning
 * a packet. Per-opcode helpers live in `att-decoders.ts`; this is a thin
 * dispatcher that falls back to the default ATT opcode-name display when
 * no decoder matches or the matched decoder reports the payload is too
 * short.
 */
