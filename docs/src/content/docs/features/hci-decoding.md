...
By default, MON messages are hidden. Use the **MON** toggle button in the [filter bar](/logscope/features/filtering/) to show them. The sidebar "HCI Packets" counter only counts real HCI protocol packets, not MON messages.

## Exporting HCI Packets

[Export](/logscope/features/export/) captured HCI packets in **Wireshark btsnoop format** for detailed protocol analysis:
